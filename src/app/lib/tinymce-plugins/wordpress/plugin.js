...
tinymce.PluginManager.add( 'wordpress', function( editor ) {
	...
	editor.on( 'preInit', function() {
		...
		if ( getUserSetting( 'editor_plain_text_paste_warning' ) > 1 ) {
	...
	editor.on( 'PastePlainTextToggle', function( event ) {
		...
		if ( event.state === true ) {
			var times = parseInt( getUserSetting( 'editor_plain_text_paste_warning' ), 10 ) || 0;

			if ( times < 2 ) {
				setUserSetting( 'editor_plain_text_paste_warning', ++times );
			}
		}
	});

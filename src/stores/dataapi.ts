...
class DataApi{
    ...
    getContentSubTypes(type, callback): void {
    ...
    addOfflineContentSubType(type, subType): void{
        let fileName = path.join(CONTENT_DIR, type+"_subtypes.json");
        let fileContent = [];
    ...
    getContent(type, subType, defaultValue, schema, callback): void {
        let key = type;
